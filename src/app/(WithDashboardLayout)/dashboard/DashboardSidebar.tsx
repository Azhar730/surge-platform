...
export function DashboardSidebar() {
  ...
  return (
    <>
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-slate-800" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="relative w-64 h-full bg-white/95 p-4 border-r border-slate-200/60">
            <div className="px-2 mb-2 flex items-center justify-between">
              <span className="font-mono text-base font-bold tracking-tighter text-slate-800 select-none">zerodraft.</span>
            ...
            <div className="mt-auto">
              <button className="px-3 py-2 text-sm text-slate-500 hover:text-slate-800 transition-colors">Log out</button>
            </div>
          </div>
        </div>
      )}
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "px-3 py-2 rounded-lg text-sm font-medium transition-colors",
                  isActive ? "bg-indigo-500 text-white" : "text-slate-600 hover:bg-slate-100"
                )}
              >
                {item.label}
              </Link>
